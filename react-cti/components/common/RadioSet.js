...
const RadioInput = ({name, label, value, checked, onChange, className, inline, disabled}) => {
    let wrapperClass = (inline ? 'radio-inline ' : 'radio ') + className + (checked ? ' checked' : '');
    let inputElem =
        <input
            type="radio"
            name={name}
...
const RadioSet = ({name, options, value, onChange, className, outerClassName, inline, disabled}) => {
    return (

...
const FilterTypeHeader = ({filterTypeKey, filterTitle, onExpandFilterClick, isExpanded, onEraseClick,  numItems,
    ...
    return (
		<div className="filter-type-header">
            ...
            <span className="filter-type-header__btn filter-type-header__btn_type_clear" onClick={onEraseClick} style={{cursor:'pointer'}} />
            <span className={"filter-type-header__item-count" + (numItems > 0 ? ' filter-type-header__item-count_has-items' : '')}>{numItems}</span>
    	</div>
    );
};

FilterTypeHeader.propTypes = {

...
const BallotBoxFileItem = ({campaignKey, item, ballotBoxFileIndex, downloadPermission , allowedToEdit , cancelUpdate,restartCurrentProcess}) => {
    ...
    function getStatus() {
        ...
        switch ( item.status ) {
            case ballotBoxFileParserStatus.didNotStart:
                return 'לא התחיל';
                break;

            case ballotBoxFileParserStatus.atWork:
				let htmlBlock = [];
                htmlBlock.push(
                    <div  key={4} className="progress">
            ...
            case ballotBoxFileParserStatus.success:
                return 'עבר בהצלחה';
                break;
			case ballotBoxFileParserStatus.error:
                return 'אירעה שגיאה';
                break;
			case ballotBoxFileParserStatus.cancelled:
                return 'בוטל';
                break;
			case ballotBoxFileParserStatus.restarted:
                return 'הפעלה מחדש';
                break;
        }
    }
    ...
    function renderDownloadButton() {
        if ( downloadPermission ) {
            return (
                <Link to={"/elections/campaigns/" + campaignKey + "/ballots/files/" + item.key + '/download'} target="_blank">
    ...
    return (
        <tr>
            ...
            <td>
				...
				:
					<span>{getStatus()}
						{(restartCurrentProcess!=null) && <i className="fa fa-undo fa-6" onClick={restartCurrentProcess} title="הפעלה מחדש" style={{cursor:'pointer'}}></i>}</span>
				}
			</td>
...
export default BallotBoxFileItem;
...
class BallotBox extends Model {
    ...
    //Get all ballot activists assignments
    public function activistsAllocationsAssignments() {
        return $this->hasMany('App\Models\ActivistsAllocations', 'ballot_box_id', 'id')
        ->withActivistsAssignmentsFullData()
        ->leftJoin('activist_roles_payments','activist_roles_payments.activists_allocations_assignment_id','=','activists_allocations_assignments.id');
    }
    // !! to delete ElectionRolesGeographical!!!
    ...
    public function scopeWithBallotBoxRole($query ,$leftJoin=true) {
    ...
    public function scopeWithActivistsAllocations($query, $leftJoin = true, $withBallotRoles = false) {
        $joinMethod = $leftJoin ? 'leftJoin' : 'join';
        $query->$joinMethod('activists_allocations', function ( $joinOn ) {
            $joinOn->on([['activists_allocations.ballot_box_id', '=', 'ballot_boxes.id']]);
        });
        if($withBallotRoles){
            $query->leftJoin('ballot_box_roles', 'ballot_box_roles.id', '=', 'activists_allocations.ballot_box_role_id');
        }
    }

    public function scopeWithElectionVotesReport($query,$electionCampaignId,$typeSourceId=false){
        $query->leftJoin('election_votes_report',function($joinOn)use($electionCampaignId,$typeSourceId){
            $joinOn->on('election_votes_report.ballot_box_id','=','ballot_boxes.id')
                   ->on('election_votes_report.election_campaign_id','=',DB::raw($electionCampaignId));

                   if($typeSourceId)
                   $joinOn->on('election_votes_report.vote_report_source_id','=',DB::raw($typeSourceId));
        });
    }
    /** 
     * Get query for calculate ballot mi shas votes.
    */
    public static function getCalculatedProbabilityShasVotesQuery(){
        return 'ROUND( (calculated_probability_shas_votes_percents * calculated_probability_total_votes_percents * ballot_boxes.voter_count)/(100*100) )';
    }
    //---------------------------
    ...
    public static function getBallotBoxIdByMiId_Activist_voter_id($miId,$voter_activist_id,$electionCampaignId){
       ...
       ->where('election_roles_by_voters.voter_id','=',DB::raw($voter_activist_id))
       ->get();

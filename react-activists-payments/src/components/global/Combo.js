...
class Combo extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (!this.props.cleanSelectedItems && nextProps.cleanSelectedItems != undefined && nextProps.cleanSelectedItems == true) {
            this.initCombo();
            if(this.props.resetCleanAll){
                this.props.resetCleanAll();
            }
        }
    ...
    itemClick(item) {
       // debugger;
        if(this.props.arrItemForReset && this.props.arrItemForReset.includes(item[this.props.itemIdProperty])){
           // this.clearCombo();
        }
    ...
    isItemDisabled(item){
        var idItem=item[this.props.itemIdProperty];
        var hashSelect=this.selectedItemsHash;
        //is selected
        if((!hashSelect || Object.keys(hashSelect).length==0) || (hashSelect && hashSelect.hasOwnProperty(idItem)))
        return false;
  
        var firstSelect=Object.keys(hashSelect)[0];
        //is select value in arr item only one 
        if(this.props.arrItemForReset && this.props.arrItemForReset.findIndex((a)=>a==firstSelect)!=-1)
        return true;
        if(this.props.arrItemForReset && (this.props.arrItemForReset.findIndex((a)=>a==firstSelect)==-1 && this.props.arrItemForReset.findIndex((a)=>a==idItem)!=-1))
        return true;
